...
<div id="main">
  <!-- Nav Item - Monitoring -->
  <li class="nav-item">
    <a class="nav-link" id="monitorLinkSidebar" href="#">
      <i id="monitorIcon" class="fas fa-fw fa-chart-line"></i>
      <span id="monitorSpan">Monitoring</span>
    </a>
  </li>
</div>
...
<script>
  $("#monitorLinkSidebar").attr("href", "monitor.php?remote=" + encodeURI(remoteId) + "&project=" + encodeURI(projectName));
  ...
  if (location.pathname == "/monitor.php"){
    $('#monitorSpan').css('color','#fff');
    $('#monitorIcon').css('color','#fff');
  }
